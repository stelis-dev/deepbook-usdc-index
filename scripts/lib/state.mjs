...
export async function loadWorkflowState() {
  return readJson(workflowMissingPath(), initialWorkflowState());
}
...
export function recordMissingBucket(pairState, startIso, reason) {
...
export function recordLiveBucketAttempt(pairState, startIso, result) {
  advanceLiveQueuedBucket(pairState, startIso);
  if (result.status !== "missing") {
    recordCoveredLiveBucket(pairState, startIso, result);
  }
  if (result.status === "missing") {
    recordMissingBucket(pairState, startIso, result.reason);
  } else {
